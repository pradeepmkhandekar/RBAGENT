...
public class QuotesAdapter extends RecyclerView.Adapter<QuotesAdapter.QuotesItem> implements IResponseSubcriber {


    Context mContext;
    List<QuoteDisplayEntity> listQuoteDisplayEntities;


    public QuotesAdapter(Context context, List<QuoteDisplayEntity> listQuoteDisplayEntities) {
        mContext = context;
    ...
    public void OnSuccess(APIResponse response, String message) throws InterruptedException {
        if (response instanceof EditQuoteResponse) {
            if (response.getStatus_Id() == 0) {
                ...
                if(customerEntity.getProductId()==7){
    ...
    public QuotesItem onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        view = LayoutInflater.from(parent.getContext()).inflate(
                R.layout.quotes_item, parent, false);
    ...
    public void onBindViewHolder(QuotesItem holder, int position) {
        ...
        final QuoteDisplayEntity quoteDisplayEntity = listQuoteDisplayEntities.get(position);
        ...
        holder.ll_editQuotes.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                new EditQuoteController(mContext).getCustomerDetails("" + quoteDisplayEntity.getID(), QuotesAdapter.this);
            }
        });
    }
